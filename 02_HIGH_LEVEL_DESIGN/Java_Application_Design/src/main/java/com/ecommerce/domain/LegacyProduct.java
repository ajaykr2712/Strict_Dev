package com.ecommerce.legacy.domain;
 ...
 * - Rich domain model
 */
@Deprecated // Use com.ecommerce.entity.Product instead
public class LegacyProduct {
    ...
    public LegacyProduct(String name, String description, BigDecimal price) {
    ...
    public LegacyProduct(String id, String name, String description, BigDecimal price,
    ...
    public LegacyProduct withPrice(BigDecimal newPrice) {
        validatePrice(newPrice);
        return new LegacyProduct(this.id, this.name, this.description, newPrice,
                          this.category, this.status, this.stockQuantity,
                          this.createdAt, LocalDateTime.now());
    }
    
    public LegacyProduct withStock(int newStock) {
        return new LegacyProduct(this.id, this.name, this.description, this.price,
                          this.category, this.status, Math.max(0, newStock),
                          this.createdAt, LocalDateTime.now());
    }
    
    public LegacyProduct withStatus(ProductStatus newStatus) {
        return new LegacyProduct(this.id, this.name, this.description, this.price,
                          this.category, newStatus, this.stockQuantity,
                          this.createdAt, LocalDateTime.now());
    }
    
    public LegacyProduct withCategory(ProductCategory newCategory) {
        return new LegacyProduct(this.id, this.name, this.description, this.price,
    ...
    public LegacyProduct reserveStock(int quantity) {
