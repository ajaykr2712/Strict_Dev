...
// Minimal event store singleton (enum based) + event model (restored in compact form)
enum NetflixEventStoreManager {
    INSTANCE;
    private final List<StreamingEvent> events = Collections.synchronizedList(new ArrayList<>());
    private final Map<String, Long> typeCounts = new ConcurrentHashMap<>();
    public void storeEvent(StreamingEvent e) { events.add(e); typeCounts.merge(e.type(),1L,Long::sum); }
    public List<StreamingEvent> getUserEvents(String userId){ return events.stream().filter(e->e.userId().equals(userId)).toList(); }
    public List<StreamingEvent> getEventsByType(String type){ return events.stream().filter(e->e.type().equals(type)).toList(); }
    public int getTotalEventCount(){ return events.size(); }
    public Map<String, Long> getEventTypeCounts(){ return new HashMap<>(typeCounts); }
}
record StreamingEvent(String eventId, String userId, String type, Map<String,Object> data) {}
...
class NetflixConfigurationManager {
    // Centralized immutable presets replacing verbose switch
    private static final Map<String, Map<String,String>> PRESETS = Map.of(
        "PROD", Map.of("max_streaming_quality","4K","cdn_servers","1000","cache_ttl","3600"),
        "DEV",  Map.of("max_streaming_quality","1080p","cdn_servers","10","cache_ttl","300"),
        "TEST", Map.of("max_streaming_quality","720p","cdn_servers","1","cache_ttl","60")
    );
    ...
    private NetflixConfigurationManager(String environment) {
        this.environment = PRESETS.containsKey(environment) ? environment : DEFAULT_ENV;
        loadConfigurations();
    }
    ...
    public static NetflixConfigurationManager getInstance(String environment) {
        if (instance == null) {
            synchronized (NetflixConfigurationManager.class) {
                if (instance == null) instance = new NetflixConfigurationManager(environment);
            }
        }
        return instance;
    }
     ...
     */
    public static NetflixConfigurationManager getInstance() { return getInstance(DEFAULT_ENV); }

    /**
     * Load configurations based on environment
     */
    private void loadConfigurations() { configurations.putAll(PRESETS.get(environment)); }

    /**
     * Get configuration value
     */
    public String getConfiguration(String key) { return configurations.get(key); }

    /**
     * Set configuration value
     */
    public void setConfiguration(String key, String value) { configurations.put(key, value); }
